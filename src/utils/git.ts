...
export class GitUtils {
   ...
   */
  async createBranch(branchName: string, baseBranch: string = 'dev'): Promise<void> {
    try {
      await this.git.checkout(baseBranch);
      await this.git.pull();
      await this.git.checkoutLocalBranch(branchName);
    } catch (error) {
      logger.error(`Failed to create branch ${branchName} from ${baseBranch}:`, error instanceof Error ? error.message : error);
      throw error;
    }
  }
