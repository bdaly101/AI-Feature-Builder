 ...
 */
export function backupFile(filePath: string, backupPath: string): void {
  try {
    if (existsSync(filePath)) {
      ensureDir(dirname(backupPath));
      copyFileSync(filePath, backupPath);
      logger.debug(`Backed up file: ${filePath} -> ${backupPath}`);
    }
  } catch (error) {
    logger.error(`Failed to backup file ${filePath}:`, error instanceof Error ? error.message : error);
    throw error;
  }
}
...
export function restoreFile(backupPath: string, originalPath: string): void {
  try {
    if (existsSync(backupPath)) {
      ensureDir(dirname(originalPath));
      copyFileSync(backupPath, originalPath);
      logger.debug(`Restored file: ${backupPath} -> ${originalPath}`);
    }
  } catch (error) {
    logger.error(`Failed to restore file ${backupPath}:`, error instanceof Error ? error.message : error);
    throw error;
  }
}
